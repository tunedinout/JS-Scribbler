...
export async function getAuthURL() {
  try {
    ...
    return {
      message:
        response?.data?.message || response?.data || 'Unable to get AUTH url',
      ...response,
    }
  }
}
...
export async function load(scribbles) {
  try {
    const response = await axios({
      url: `${API_HOST}/${endpoints.load}`,
      method: 'POST',
    ...
    return {
      message: response?.data?.message || response?.data || `unable to sync`,
      ...response,
    }
  }
}
