...
import { postMessageLoadWorker } from '@src/workers/api-workers'
...
export const AuthProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    fetchMe(controller.signal)
      ...
      .then(() => {
        setIsLoggedIn(true)
        postMessageLoadWorker({ type: Events.load, isLoggedIn: true })
      })
      .catch((error) => {
        log(`error:`, error)
        setIsLoggedIn(false)
        setUserInfo(null)
        postMessageLoadWorker({ type: Events.load, isLoggedIn: false })
      })
