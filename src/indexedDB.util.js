import { Events, IndexedDbName, IndexedDbScribbleStore } from './constants'
...
export async function openIndexedDBForScribbles() {
  return new Promise((resolve, reject) => {
    const idb =
      typeof window !== 'undefined' ? window.indexedDB : self.indexedDB
    const request = idb.open(IndexedDbName, 1)
...
export async function loadScribbles() {
  const log = logger(`loadScribbles`)
  const db = await openIndexedDBForScribbles()
  const transaction = db.transaction([IndexedDbScribbleStore], 'readonly')
  const scribblersObjectStore = transaction.objectStore(IndexedDbScribbleStore)
  ...
  return new Promise((resolve, reject) => {
    request.onsuccess = () => {
      // channel.postMessage({ type: Events.load })
      const result = request.result
      log(`loaded scribbles`, result)
      resolve(result)
    }
    request.onerror = () => {
